...
ReplaceTracedLineCommand::ReplaceTracedLineCommand(digitqt::core::Measurement &measurement,
                                                   size_t lineIndex,
  ...
  setText(QCoreApplication::translate("ReplaceTracedLineCommand", "Edit traced fringe"));
}
...
void ReplaceTracedLineCommand::redo() {
  ...
  if (m_lineIndex >= lines.size())
    return;
  lines[m_lineIndex].points = m_after;
}
...
void ReplaceTracedLineCommand::undo() {
  ...
  if (m_lineIndex >= lines.size())
    return;
  lines[m_lineIndex].points = m_before;
}
