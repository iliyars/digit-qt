...
Rectangle::Rectangle(double width, double height, double centerX,
                     double centerY, double rotationDegrees,
                     TypeLimits typeLimits, CoordinateSystem spatialSystem,
...
Rectangle::Rectangle(const Point &p0, const Point &p1, const Point &p2,
                     TypeLimits typeLimits, CoordinateSystem spatialSystem,
                     NormalizationState normState)
    : cosRot_(0.0), sinRot_(0.0) {
...
bool Rectangle::isOnContour(const Point &point, double tolerance) const {
  ...
  if (std::abs(local.x) > enlargedHalfWidth ||
      std::abs(local.y) > enlargedHalfHeight) {
  ...
  // If diminished rectangle is degenerate (tolerance >= half-width or
  // half-height), and we're inside enlarged, then we're on the contour
  ...
  return (std::abs(local.x) > diminishedHalfWidth ||
          std::abs(local.y) > diminishedHalfHeight);
}
...
void Rectangle::EnumerateHandles(std::vector<HandleDesc> &out) const {
  ...
  double rotHandleOffset =
      std::max(boundsRadius * 0.2, 20.0);  // 20.0 = min offset
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    Point normal{cornerPts[i].x - center_.x, cornerPts[i].y - center_.y};
    ...
    out.push_back(
        HandleDesc{HandleType::CornerResize, i, cornerPts[i], normal});
  }
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    int nextIdx = (i + 1) % 4;
    Point edgeMid{(cornerPts[i].x + cornerPts[nextIdx].x) / 2.0,
                  (cornerPts[i].y + cornerPts[nextIdx].y) / 2.0};

    // Normal = perpendicular to edge (outward)
    Point edge{cornerPts[nextIdx].x - cornerPts[i].x,
...
void Rectangle::ApplyHandleDrag(const HandleDesc &handle,
  ...
  switch (handle.type) {
    ...
    case HandleType::Rotate: {
      ...
      double angleRad =
          std::atan2(dy, dx);  // atan2(x,y) for +Y = up in shape space
    ...
    case HandleType::CornerResize: {
      ...
      double deltaWorldX =
          newCenterLocalX * cosRot_ - newCenterLocalY * sinRot_;
      double deltaWorldY =
          newCenterLocalX * sinRot_ + newCenterLocalY * cosRot_;
    ...
    case HandleType::EdgeResize: {
      ...
      // Determine edge orientation: 0,2 = horizontal (top/bottom), 1,3 =
      // vertical (right/left)
