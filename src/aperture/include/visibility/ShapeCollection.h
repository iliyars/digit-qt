...
public:
   ...
   */
  void notifyShapeModified() { version_++; }
   ...
   */
  bool hasAnyExternal() const { return !external_.empty(); }
   ...
   * bounds are necessarily visible. You must still call isVisible() for each
   * point.
   *
   * The ROI guarantees:
   * - All points OUTSIDE the ROI are invisible (safe to skip)
   * - All points INSIDE the ROI *might* be visible (must check with
   * isVisible())
   *
   ...
   * shapes.addInternal(std::make_unique<Ellipse>(30, 50, 70, 50)); //
   * Obstruction
   *
