...
public:
   ...
   */
  static VisibilityMask Build(const ShapeCollection &shapes, int width,
                              int height) {
    ...
    for (int y = 0; y < height; ++y) {
      for (int x = 0; x < width; ++x) {
        int index = y * width + x;
        mask.data[index] =
            checker.isVisible({static_cast<double>(x), static_cast<double>(y)})
                ? 1
                : 0;
      }
    }
   ...
   */
  static VisibilityMask Build(const ShapeCollection &shapes, int width,
                              int height, uint64_t imgVersion) {
