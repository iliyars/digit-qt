 ...
 *     if (!shape.isInside(point)) {
 *         visible = false;  // Outside boundary (but continue - APERTURE can override)
 *         break;
 *     }
 * }
 * ```
 ...
 * shapes.addExternal(std::make_unique<Ellipse>(100, 100, 0, 0));  // Main aperture
 * shapes.addInternal(std::make_unique<Ellipse>(20, 20, 0, 0));    // Central obstruction
 * 
 * // Create checker
 ...
 * Point opened{2, 10};     // Inside APERTURE ? true (overrides INTERNAL location!)
 * Point outside{110, 0};   // Outside EXTERNAL ? false
 * 
...
private:
    const ShapeCollection& shapes_;  ///< Shape collection reference (must outlive checker)
    mutable Stats stats_;            ///< Performance counters (mutable for const isVisible())
};
