 ...
 * compatibility and **system-agnostic accessors** (minX/maxX/minY/maxY) for
 * clarity.
 *
 * **Recommended:** Use `minX()`, `maxX()`, `minY()`, `maxY()` and
 * `fromMinMax()` for code that works correctly with both coordinate systems.
 *
...
public:
  ...
  /**
   * @brief Default constructor - creates empty bounds at origin in SCREEN
   * coordinates
   *
  ...
  Bounds(double l, double t, double r, double b)
      : left(l),
        ...
        right(r),
        bottom(b),
        spatialSystem(CoordinateSystem::screen()) {}
   ...
   * @see fromCorners()
   */
  static Bounds fromCenterAndSize(
      const Point &center, double width, double height,
      CoordinateSystem sys = CoordinateSystem::screen()) {
    double halfW = width / 2.0;
    double halfH = height / 2.0;
    return {center.x - halfW, center.y - halfH, center.x + halfW,
            center.y + halfH, sys};
  }
   ...
   */
  static Bounds fromMinMax(double min_x, double min_y, double max_x,
                           double max_y,
                           CoordinateSystem sys = CoordinateSystem::screen()) {
    if (sys.isScreen()) {
   ...
   * @see maxY(), minX(), maxX()
   */
  double minY() const { return spatialSystem.isScreen() ? top : bottom; }
   ...
   * @see minY(), minX(), maxX()
   */
  double maxY() const { return spatialSystem.isScreen() ? bottom : top; }
   ...
   * @see fromCenterAndSize()
   */
  Point center() const { return {(left + right) / 2.0, (top + bottom) / 2.0}; }
   ...
   * @see isEmpty()
   */
  void clear() { left = top = right = bottom = 0.0; }
   ...
   * @endcode
   */
  void shift(const Point &offset) { shift(offset.x, offset.y); }
  ...
  Point clamp(const Point &point) const {
    return {std::clamp(point.x, left, right), std::clamp(point.y, top, bottom)};
  }
