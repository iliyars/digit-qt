 ...
 * 
 * The Point struct provides fundamental 2D coordinate operations used throughout
 * the ApertureCore geometry module. It replaces the legacy XYPoint class with
 * modern C++17 features while maintaining compatibility.
 * 
 * ## Key Features
 ...
 * 
 * The Point struct is designed as an aggregate type, allowing brace initialization
 * and guaranteed copy elision. All methods are constexpr where possible for
...
struct Point {
     ...
     * 
     * @warning Uses exact floating-point equality - consider isNear() for tolerance
