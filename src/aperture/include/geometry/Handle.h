 ...
 * 
 * Returned by Shape::EnumerateHandles() to describe each interactive control point.
 * 
 * **This is NOT a runtime handle object** - it's a lightweight descriptor.
 * BoundsHandler converts these to RuntimeHandle with screen positions, hover state, etc.
 * 
 * ### Coordinate Space:
...
struct HandleDesc {
    ...
    int index = -1;                      ///< Shape-specific index (-1 for Move/Rotate, vertex/corner/edge index otherwise)
...
struct DragContext {
    ...
    bool shiftKey = false;     ///< Shift modifier: constrain/snap (e.g. preserve aspect ratio)
