...
public:
   ...
   */
  Ellipse(double semiMajorAxis, double semiMinorAxis, double centerX,
          double centerY, double rotationDegrees = 0.0,
   ...
   * - **6+ points**: Least squares ellipse fit (algebraic distance
   * minimization)
   *
   * ## Algorithm Details
   *
   ...
   *
   * - Fitzgibbon, Pilu, Fisher: "Direct Least Square Fitting of Ellipses"
   * (1999)
   ...
   *
   * // Works with rotation
   * assert(ellipse.isOnContour(ellipse.center(), tolerance) == false);  //
   * Center not on edge
   * @endcode
   *
   ...
   *
   * Transforms: (a_norm, b_norm, center_norm) = (a/radius, b/radius,
   * (center-origin)/radius)
   *
   * @code{.cpp}
