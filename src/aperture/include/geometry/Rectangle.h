 ...
 * ### 3. Corner Points
 *
 * Extract corner points in consistent order (Top-Left, Top-Right, Bottom-Right,
 * Bottom-Left):
 *
 * @code{.cpp}
...
public:
   ...
   * @param rotationDegrees Rotation angle in degrees (counter-clockwise from +X
   * axis)
   ...
   *
   * Creates a rectangle with specified dimensions centered at (centerX,
   * centerY). Optional rotation is applied counter-clockwise (in MATH
   * coordinates) or clockwise (in SCREEN coordinates due to inverted Y-axis).
   *
   * ## Examples
   *
   * @code{.cpp}
   ...
   * @endcode
   *
   * @note Width and height must be positive (not enforced, but required for
   * valid geometry)
   ...
   * approximately stepSize apart. Points are in world coordinates (rotation
   * applied).
   *
   * The contour forms a closed loop (first and last points are
   * close/identical).
   *
   * @code{.cpp}
   * Rectangle rect(100.0, 50.0, 0.0, 0.0);
   ...
   *
   * Returns corner points in clockwise order (when viewed in SCREEN
   * coordinates):
   * - [0] Top-Left
   * - [1] Top-Right
   * - [2] Bottom-Right
   ...
   * - CornerResize: Resize from opposite corner (Shift=preserve aspect,
   * Alt=from center)
