 ...
 * ## Overview
 *
 * Shape defines the common interface for all 2D geometric shapes in
 * ApertureCore:
 ...
 *
 * Each shape has a visibility type that controls how it affects point
 * visibility:
 ...
 *
 * // Use polymorphically
 * std::unique_ptr<Shape> shape = std::make_unique<Ellipse>(10.0, 10.0, 0.0,
 * 0.0);
 * @endcode
 *
 ...
 * |-------|------|--------|---------|
 * | Visibility | TypeLimits | EXTERNAL, INTERNAL, APERTURE | How shape affects
 * visibility | | Spatial | CoordinateSystem | SCREEN, MATH | Y-axis direction |
 * | Normalization | NormalizationState | MEASURING, NORMALIZED | Coordinate
 * scale |
 *
...
public:
   ...
   * @param stepSize Maximum distance between consecutive points (in current
   * units)
   ...
   * Points should form a closed contour (first and last points close or
   * identical).
   *
   * Typical uses:
   ...
   * @note Points exactly on boundary should return true with any positive
   * tolerance
   ...
   *
   * Common use in optics: normalize pupil to unit circle for aberration
   * analysis.
   *
   * @code{.cpp}
   ...
   *
   * @note Calling normalize() on already-normalized shape may give unexpected
   * results
   ...
   *
   * **Shapes expose handles, BoundsHandler interprets interaction, Commands
   * commit geometry.**
   *
   ...
   * @endcode
   *
   * @param out Vector to receive handle descriptors. Existing contents are
   * preserved. Append new handles, don't clear vector.
   *
   ...
   *
   * @code{.cpp}
   * void Rectangle::ApplyHandleDrag(const HandleDesc& handle, const
   * DragContext& drag) { switch (handle.type) { case HandleType::Move:
   *             // Simple translation
   ...
   *             } else {
   *                 // Resize from opposite corner
   *                 ResizeFromCorner(handle.index, drag.deltaWorld,
   * drag.shiftKey);
   *             }
   *             break;
   *
   ...
   * @endcode
   *
   * @param handle Frozen handle descriptor from drag start (IMMUTABLE during
   * drag)
   ...
   * @warning Do NOT snap to grid - BoundsHandler applies snapping before
   * calling this
   *
   ...
   */
  virtual void ApplyHandleDrag(const HandleDesc &handle,
   ...
   */
  CoordinateSystem getSpatialSystem() const { return spatialSystem_; }
  ...
  virtual void transformToSystem(const CoordinateSystem &targetSystem) {
    ...
    if (height <= 0.0) {
      throw std::invalid_argument(
          "Shape::transformToSystem: reference height must be set for "
   ...
   */
  NormalizationState getNormalizationState() const { return normState_; }
   ...
   *
   * @warning This does NOT transform coordinates, only updates the tag!
   *          To actually normalize/denormalize, use normalize() or
   * denormalize().
   *
   * Use this when:
   ...
   */
  void setNormalizationState(NormalizationState state) { normState_ = state; }
...
protected:
  TypeLimits typeLimits_{
      TypeLimits::EXTERNAL};  ///< Visibility behavior
                              ///< (EXTERNAL/INTERNAL/APERTURE)
  CoordinateSystem spatialSystem_{
      CoordinateSystem::screen()};  ///< Spatial coordinate system (SCREEN/MATH)
  NormalizationState normState_{
