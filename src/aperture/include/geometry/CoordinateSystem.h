...
private:
     ...
     */
    double getReferenceHeightForConversion(const CoordinateSystem& targetSystem) const {
        ...
        if (height <= 0.0) {
            throw std::invalid_argument(
                "CoordinateSystem: referenceHeight must be set for Y-axis conversion. "
    ...
    double referenceHeight_;  ///< Reference height for Y-axis conversions (image height in pixels)
};
